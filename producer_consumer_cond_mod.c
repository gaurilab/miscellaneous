...
void *puta(void*arg)
{
	...
	while (1)
	{
		...
		if (!(j--))
		{
			while(i--)
			{
				printf("\n %s 0x%x producer thread going to the evaluate the condition  ",__func__,pthread_self());
				sleep(1);
			}
		}
#endif
		...
		//if( ((queue.rear+1)%SIZE )== (queue.front%SIZE))
...
void *eata(void*arg)
{
	...
	{
		pthread_mutex_lock(&queue.m);
		printf("\n %s 0x%x consumer thread going to the evaluate the condition  ",__func__,pthread_self());
		...
		sleep(1);
		//if(((queue.rear)%SIZE ) == (queue.front%SIZE))
